...
> 适用版本：v0.1.0 起（当前最新：v0.2.5）。本文档描述**本地打包**与 **GitHub Release 发布**的完整流程。
...
| 版本 | 日期 | 内容 | 产物（bin/） |
|---|---|---|---|
| v0.2.5 | 2026-08-12 | 修复配置迁移测试与路由测试配置，恢复发布构建测试通过 | `Binvia-0.2.5-macos-arm64-x86_64.dmg` / `.tar.gz` |
| v0.2.4 | 2026-08-12 | 修复服务重复启动、请求体上限、模型权限、流式 token 统计与错误 JSON 编码 | `Binvia-0.2.4-macos-arm64-x86_64.dmg` / `.tar.gz` |
