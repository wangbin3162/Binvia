...
Binvia：本地 AI 供应商聚合路由网关（macOS）。在本地端口提供 OpenAI 兼容 API（`/v1/chat/completions`、`/v1/models` 等），连接 DeepSeek / 腾讯 CodeBuddy / Google Antigravity 等上游，供 Claude Code、Codex、各类 SDK 通过 api-key 调用，带请求监控与菜单栏 GUI。纯 Swift + 原生 POSIX socket，零外部依赖（Package.swift 无任何第三方依赖）。
...
- 测试（全部套件）：`make test` 即 `swift run BinviaCheck`
  - 测试是单个可执行文件（`Sources/BinviaCheck/main.swift`），内含极简断言框架；**无法按用例过滤**，想只跑某套件需临时注释入口处的 `await run(...)` 行。
  - 退出码：0=全部通过，1=存在失败断言。
...
- GUI 无界面自检：`swift run BinviaApp --smoke-test`
  - 注意：`swift run` 可能被权限分类器拦截，可改跑 `./.build/debug/BinviaApp --smoke-test`
- 打包（release 构建 + 自检 + 拷贝到 bin/）：`make release` 即 `./Scripts/build.sh`，产物 `bin/Binvia.app`、`bin/BinviaServer`、`bin/BinviaCLI`
...
5 个 target，全部开启 `StrictConcurrency`：

- **BinviaCore**（库）：全部业务逻辑——Provider 协议/注册表、路由、认证、配置、监控、上游 HTTP 客户端、本地 HTTP 服务器。GUI/CLI 只依赖它。
- **BinviaServer**（可执行）：服务器入口 `main.swift`：加载配置 → `ProviderCatalog.registerAll()` → `RouteHandler` → `HTTPServer` 常驻。
...
- **Provider 协议 / 描述符 / 注册表**（`Provider/`）：
  - `Provider.chat` **始终返回 `AsyncThrowingStream<Data>`**（SSE chunk 或整段 JSON body），由上层决定透传（streaming）或聚合（non-streaming）。
  - `ProviderDescriptor` 携带静态元数据（id / alias / 模型目录 / baseURL）+ `makeProvider` 工厂闭包。
  - `ProviderRegistry.shared` 是 NSLock 单例，支持 alias 解析。
  - **新增供应商**：在 `Providers/` 下新建目录，实现 `Provider` + 描述符，最后在 `ProviderCatalog.swift` 加一行 `register(...)`。README 表格同步更新。
- **Router**（`Router/Router.swift`）：解析 `provider/model`、`alias/model`、裸模型名（静态目录消歧：模型名以 provider id/alias 前缀开头者优先，仍歧义取字母序）。
- **Networking/**：
  - `ProviderHTTPClient`：统一上游请求。`data(for:retryPolicy:)` 指数退避重试（仅幂等方法 + 408/429/5xx，尊重 `Retry-After` 封顶 60s）；`stream(for:)` 逐字节透传、非 2xx 把错误 body 透传（反向代理语义）；`streamThrowing(for:)` 非 2xx 抛 `ProviderError.upstreamError`（供 key 轮换等判断握手场景）。
  - `SSEParser`：跨 chunk 累积、空行切分事件。
  - `SSEJSONAggregator`：把强制流式上游的 SSE 聚合成 OpenAI 单 JSON（供 `stream=false` 客户端）。
  - `ModelCache`：actor，300s TTL，缓存 `/v1/models` 动态模型。
- **HTTPServer + RouteHandler**（`Server/`）：手写 POSIX socket 服务器（poll 500ms 循环，每连接独立 Task，10s 读超时，无外部框架）。`RouteHandler` 分发 4 个端点：`GET /v1/health`、`GET /v1/models`、`POST /v1/chat/completions`、`GET /v1/usage`。`HTTPServer.setHandler()` 支持运行时热替换 handler（GUI 热更新依赖此机制）。流式响应不设 Content-Length，靠连接关闭标识结束。
- **Config**（`Config/`）：`ConfigStore` 读写 `~/.config/binvia/config.json`（`BINVIA_CONFIG` 覆盖），0600 权限，snake_case 编解码。`ProviderConfig` 对旧格式缺字段（如 `apiKeys`）有默认值回退。凭据解析：config 优先，回退 `<PROVIDER>_API_KEY` / `<PROVIDER>_ACCESS_TOKEN` 等环境变量（`RouteConfig.credential(for:)`、`apiKeys(for:)`）。
- **Auth**（`Auth/APIKeyAuthenticator.swift`）：Bearer / x-api-key；未配置 apiKeys 时允许匿名（开发模式）。恒有效 env key：`BINVIA_API_KEY` / `ROUTER_API_KEY` / `OMNIROUTE_API_KEY`。
- **Monitoring**（`Monitoring/RequestLogger.swift`）：内存日志，上限 10k 条，按 provider/model 聚合用量，`/v1/usage` 输出。

## 上游流式差异（重要语义）

- DeepSeek 原生支持流式+非流式。
- CodeBuddy 上游**强制 `stream=true`**（非流式会被 400 拒绝），非流式客户端由 provider 内部用 `SSEJSONAggregator` 聚合（`CodeBuddyCNProvider.swift:104-119`）。新增 OAuth 类供应商时遵循这一「强制流式上游 → 非流式客户端」模式。
...
- 网络测试两种 mock：`URLProtocolMock`（URLSession 层 mock，测重试策略）+ 本地真实 `HTTPServer` 当 mock 上游（测 DeepSeek 集成、SSE 透传/聚合）。
...
- 设计约束：零外部依赖，新增功能优先用 Foundation / AppKit 实现，不要轻易引入第三方包。
